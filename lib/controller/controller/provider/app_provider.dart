...
class AppProvider extends ChangeNotifier {
  ...
  getProductFromCategory(String categoryId) async {
    spacificProducts = await FireStoreHelper.fireStoreHelper
  ...
  getAllCartProducts() async {
    ...
    for (int i = 0; i < cartProducts.length; i++) {
      a += cartProducts[i].price * cartProducts[i].numOfCount;
    }
    totalPrice = a;
    notifyListeners();
  }
  ...
  addFavoiriteProduct(Product product) async {
    Product product1 =
        await FireStoreHelper.fireStoreHelper.getOneProduct(product.id);
  ...
  getAllFavoriteProducts() async {
    FavoriteProducts =
        await FireStoreHelper.fireStoreHelper.getAllFavoriteProducts();
    notifyListeners();
  }
  ...
  Widget buildBackground(
      int index, double width, Function function, String categoryId) {
    return ClipPath(
      ...
      child: GestureDetector(
        onTap: () {
          getProductFromCategory(categoryId);
          RouterHelper.routerHelper
              .routingToSpecificWidget(const SpisificProductScreen());
        },
        child: Container(
